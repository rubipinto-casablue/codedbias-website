...
========================================================= */
  ...
  /* -----------------------------
     ...
     - KEEP YOUR ORIGINAL SELECTORS (do not change classes)
     - Adds /?panel=slider support by clicking the existing CTA
  ----------------------------- */
  ...
  function applyHomePanelIntent(container = document) {
    ...
    // Defer so Webflow/Barba/GSAP layout is settled and click handlers exist
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        setTimeout(() => {
          ...
          // Trigger the same transition as the user click
          btn.click();

          // Optional: remove param so future home visits start on intro again
  ...
  function setupHomePanels(container = document) {
    ...
    // IMPORTANT: keep exactly as your stable version (no class changes)
...
========================================================= */
    ...
    transitions: [{
      ...
      async after(data) {
        try {
          ...
          // NEW: if landing on Home with ?panel=slider, auto-trigger the CTA
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // NEW: first load support for /?panel=slider
  ...
  window.addEventListener("pageshow", (evt) => {
    ...
    // NEW: bfcache restore support for /?panel=slider
    applyHomePanelIntent(document);
